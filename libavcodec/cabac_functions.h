...
// Globals
extern const uint8_t next_state_mps[128];
extern const uint8_t next_state_lps[128];
extern const uint8_t lpst_table[64][4];
extern const uint8_t renorm_table[32];


// Encoder functions
void cabac_start(cabac_data_t *data, uint8_t *buf, int size); //initialisation in ff_hevc_cabac_init
void cabac_encode_bin(cabac_data_t *data, uint32_t bin_value);
void cabac_encode_bin_bypass(cabac_data_t *data, uint32_t bin_value);
void cabac_encode_bins_bypass(cabac_data_t *data, uint32_t bin_values, int num_bins);
...
/*void cabac_write_coeff_remain(HEVCContext *s, uint32_t symbol,
                              uint32_t r_param);
void cabac_write_coeff_remain_encry(HEVCContext *s, const uint32_t symbol,
		const uint32_t r_param, int32_t base_level);
void cabac_write_ep_ex_golomb( HEVCContext *s,uint32_t symbol, uint32_t count);
void cabac_write_unary_max_symbol(HEVCContext *s, uint32_t symbol, int32_t offset,
                                  uint32_t max_symbol);
void cabac_write_unary_max_symbol_ep(HEVCContext *s, unsigned int symbol, unsigned int max_symbol);
*/

// Macros
#define CTX_STATE(ctx) ((ctx)->uc_state >> 1)
#define CTX_MPS(ctx) ((ctx)->uc_state & 1)
#define CTX_UPDATE_LPS(ctx) { (ctx)->uc_state = next_state_lps[ (ctx)->uc_state ]; }
#define CTX_UPDATE_MPS(ctx) { (ctx)->uc_state = next_state_mps[ (ctx)->uc_state ]; }
